        ...
        >
          <div v-for="(item,i) in menuList" :key="i">
                  <el-submenu :index="item.id + ''" v-if="item.children.length != 0">
                    ...
                    <el-menu-item :index="val.path + ''" v-for="val in item.children" :key="val.name" @click="saveNavStatus(val.path)">
                         <i :class="val.icon"></i>
                        <span>{{val.name}}</span>
                    </el-menu-item>
                  </el-submenu>
                  <el-menu-item :index="item.path" @click="saveNavStatus(item.path)" v-else > 
                      <i :class="item.icon"></i>
